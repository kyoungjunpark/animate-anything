...
from models.layerdiffuse_VAE import LatentSignalEncoder, SignalEncoder, SignalEncoder2, ImageReduction, \
    ...
    CompactImageReduction, CompactSignalEncoder3, FFTConv1DLinearModel, \
...
from models.pipeline_infrared_only import MaskStableVideoDiffusionPipeline
...
def load_primary_models(pretrained_model_path, fps, frame_step, n_input_frames, width, height, eval=False):
    ...
    signal_encoder3 = CompactSignalTransformer2(input_size=CHIRP_LEN, frame_step=frame_step, n_input_frames=n_input_frames+1, target_h=height // 8, target_w=width // 8)
    ...
    video_encoder = VideoEncoder(target_h=height // 8, target_w=width // 8)
    video_encoder_hidden = VideoEncoderHidden(target_h=height // 8, target_w=width // 8
...
def finetune_unet(accelerator, pipeline, batch, use_offset_noise,
                  ...
                  camera_fourier, tx_fourier, img1, img2, final_encoder, n_input_frames, pose, image_processor,
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("infrared_real_ours_ink_2skip_1input_10fps")
                    ...
                    with accelerator.autocast():
                        ...
                        if global_step < 10:
                            eval(pipeline, vae_processor, sig1, sig2, sig3, video_encoder, video_encoder_hidden,
                                 camera_fourier, tx_fourier, img1, img2, final_encoder, validation_data, out_file,
                                 global_step, preview=True)
...
def eval(pipeline, vae_processor, sig1, sig2, sig3, video_encoder, video_encoder_hidden, camera_fourier, tx_fourier,
         img1, img2, final_encoder, validation_data, out_file, index, forward_t=25, preview=True):
    vae = pipeline.vae
    device = vae.device
    dtype = vae.dtype
...
def eval_fid_fvd_videomae(evaluator, test_dataloader, pipeline, vae_processor, sig1, sig2, sig3, video_encoder,
                          video_encoder_hidden, camera_fourier, tx_fourier, img1, img2, final_encoder, validation_data,
                          out_file, flow_model, pose, index, forward_t=25, preview=True):
    vae = pipeline.vae
    device = vae.device
    dtype = vae.dtype
    fid_results = []
...
def main_eval(
    ...
    sig3, video_encoder, video_encoder_hidden, camera_fourier, tx_fourier, img1, img2 = load_primary_models(
        pretrained_model_path, validation_data.n_sample_frames, validation_data.frame_step,
        validation_data.n_input_frames,
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        eval(pipeline, vae_processor, sig1, sig2, sig3, camera_fourier, tx_fourier, img1, img2, validation_data,
             out_file,
             None)
