...
def load_primary_models(pretrained_model_path, fps, frame_step, n_input_frames, width, height, eval=False):
    ...
    image_encoder = CompactImageReduction2(input_dim=4, frame_step=frame_step, n_input_frames=n_input_frames,
                                           target_h=height // 8, target_w=width // 8,
                                           encoder_hidden_dim=encoder_hidden_dim)

    image_encoder2 = CompactImageReduction(input_dim=4, frame_step=frame_step, n_input_frames=n_input_frames,
                                           target_h=height // 8, target_w=width // 8)
    ...
    signal_encoder = CompactSignalTransformer2(input_size=CHIRP_LEN, output_dim=1, frame_step=frame_step,
    ...
    signal_encoder2 = CompactSignalEncoder3_2(signal_data_dim=CHIRP_LEN, output_dim=1, fps=fps, frame_step=frame_step,
                                              target_h=height // 8, target_w=height // 8)
    signal_encoder3 = CompactSignalTransformer2(input_size=CHIRP_LEN, output_dim=1, frame_step=frame_step,
                                                n_input_frames=n_input_frames, target_h=height // 8,
                                                target_w=width // 8)
...
def finetune_unet(accelerator, pipeline, batch, use_offset_noise,
                  ...
                  img2, n_input_frames, pose, image_processor,
                  flow_model, P_mean=0.7, P_std=1.6):
    pipeline.vae.eval()
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process:
        ...
        if not os.path.exists("fvd_real_infrared_ink.stat"):
            real_videos = []
            ...
            evaluator.save_real_stats("fvd_real_infrared_ink.stat")
            # # Shape: (1, T, C, H, W)
            del real_videos
        else:
            evaluator.load_real_stats("fvd_real_infrared_ink.stat")
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("vanila_real_new_ink_2skip_5input_10fps")
            ...
            if accelerator.sync_gradients:
                ...
                if should_sample(global_step, validation_steps, validation_data) and accelerator.is_main_process:
                    ...
                    with accelerator.autocast():
                        ...
                        if global_step < 10:
                            eval(pipeline, vae_processor, sig1, sig2, sig3, camera_fourier, tx_fourier, img1, img2,
                            ...
                            fvd = eval_fid_fvd_videomae(evaluator, test_dataloader, pipeline, vae_processor,
                                                        ...
                                                        validation_data, out_file, flow_model, pose, global_step)
                        elif global_step >= 8000:
                            eval(pipeline, vae_processor, sig1, sig2, sig3, camera_fourier, tx_fourier, img1, img2,
                                 validation_data, out_file, global_step, preview=False)
...
def eval(pipeline, vae_processor, sig1, sig2, sig3, camera_fourier, tx_fourier, img1, img2, validation_data, out_file,
    ...
    for image in sorted(validation_data.prompt_image):
        ...
        sec_to_skips = [1, 2]
        for sec_to_skip in sec_to_skips:
            image_replaced = image.split("/")[-1].replace("rgb",
                                                          str(sec_to_skip) + "sec_" + str(index) + "_rgb").replace(
                '.mp4', '.gif')

            target_file = out_file + "pred_" + image_replaced
            origin_file = out_file + "origin_" + image_replaced
            origin_inf_file = out_file + "origin_inf_" + image_replaced
            ...
            frames = vr.get_batch(frame_range[sec_to_skip * 8:validation_data.num_frames + sec_to_skip * 8])
            ...
            for i in range(frames.shape[0]):  # Iterate over the batch
                ...
                pil_image = pil_image.resize((validation_data.width, validation_data.height))  # Resize to 64x64
                pil_images.append(pil_image)
            # video = rearrange(frames, "f h w c -> f c h w")
            # video = transform(video)
            # video = normalize_input(video)
            # Convert PIL images back to NumPy arrays for saving
            origin_frames = [np.array(pil_image) for pil_image in pil_images]
            ...
            # Convert PIL images back to NumPy arrays for saving
            origin_inf_frames = [np.array(pil_image) for pil_image in pil_infrared_images]
            ...
            if preview:
                ...
                imageio.mimwrite(origin_file, origin_frames, duration=int(1000 / fps), loop=0)
                imageio.mimwrite(origin_file.replace('.gif', '.mp4'), origin_frames, fps=fps)

                imageio.mimwrite(origin_inf_file, origin_inf_frames, duration=int(1000 / fps), loop=0)
                imageio.mimwrite(origin_inf_file.replace('.gif', '.mp4'), origin_inf_frames, fps=fps)

                # wandb.log({image: wandb.Video(target_file.replace('.gif', '.mp4'), caption=target_file.replace('.gif', '.mp4'), format="mp4")})

                wandb.log({
                    '(origin)' + str(sec_to_skip) + "sec" + image: wandb.Video(origin_file.replace('.gif', '.mp4'),
                                                    caption=origin_file, format="mp4"),
                    '(origin inf)' + str(sec_to_skip) + "sec" + image: wandb.Video(origin_inf_file,
                                                        caption=origin_inf_file,
                                                        format="mp4"),

                    '(pred)' + str(sec_to_skip) + "sec" + image: wandb.Video(target_file.replace('.gif', '.mp4'),
                                                  caption=target_file, format="mp4"),
                })
            else:
                fps = validation_data.get('fps', 8)
                imageio.mimwrite(target_file, video_frames, duration=int(1000 / fps), loop=0)
...
def eval_fid_fvd_videomae(evaluator, test_dataloader, pipeline, vae_processor, sig1, sig2, sig3, camera_fourier,
                          tx_fourier, img1, img2, final_encoder, validation_data, out_file, flow_model, pose, index,
    ...
    for step, batch in enumerate(tqdm(test_dataloader)):
        ...
        pil_images = torch.stack(
            [torch.from_numpy(np.array(img.resize((validation_data.width, validation_data.height)))) for img in
             pil_images])
        ...
        if pil_images.size(0) < video_frames.size(0):
            ...
            repeated_elements = last_element.repeat(video_frames.size(0) - pil_images.size(0), 1, 1,
                                                    1)  # Repeat 3 times
    ...
    skeleton_distance = eval_skeleton_distance(pose, videos1[0], videos2[0])

    wandb.log({"fvd": score_fvd})
    wandb.log({"optical flow": optic_flow})
    wandb.log({"skeleton distance": skeleton_distance})

    return score_fvd


def eval_skeleton_distance(pose, video1, video2):
    skeletons1, _ = process_skeleton(pose, video2)
    skeletons2, _ = process_skeleton(pose, video1)
...
def eval_optical_flow(flow_model, real_videos, fake_videos, num_frames=8):
    ...
    with torch.no_grad():
        for video_idx in range(real_videos.size(0)):
            ...
            trajs_real = run_model(flow_model, real_video, N, None, str(video_idx) + "real",
                                   mask_final)  # torch.Size([1, 8, 3, 360, 640])
